...
function product_image_upload($file){
    $image = new Image($file);
    $image->setSize(0, SETTINGS['post_max_size']);
    $image->setDimension(10000,10000);
    $image->setMime(array('jpeg', 'gif', 'png', 'jpg'));
    if(!file_exists('media/products/images/')){
        mkdir('media/products/images/');
    }
    $image->setLocation('media/products/images/');
    //Upload
    if($image){
        $upload = $image->upload();
        if($upload){
...
function purchase_verify($purchase_code, $email, $url){
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://www.oncodes.co/api/v1/envato/purchase_verify");
...
function translate_term($term, $lang=false){
    if (get_setting('enable_multiple_languages')) {
        ...
        if (defined('LANG_TERMS')) {
            if (isset(LANG_TERMS[$term])) {
                return LANG_TERMS[$term];
            }else{
                return $term;
            }
        }else{
        ...
        if (stripos($contentType, 'text/html') !== false) {
            ...
            for ($i = 0; $i < $metas->length; $i++) {
                ...
                if ($meta->getAttribute('property') == 'og:image') {
                    $data['image'] = app('purify')->xss_clean(clean($meta->getAttribute('content')));
                }else if($meta->getAttribute('property') == 'twitter:image'){
                    $data['image'] = app('purify')->xss_clean(clean($meta->getAttribute('content')));
                }
                if ($meta->getAttribute('property') == 'og:price') {
                    $data['price'] = app('purify')->xss_clean(clean($meta->getAttribute('content')));
                }
                if ($meta->getAttribute('property') == 'og:quantity') {
                    $data['amount'] = app('purify')->xss_clean(clean($meta->getAttribute('content')));
                }
                if ($meta->getAttribute('property') == 'og:id') {
                    $data['id'] = app('purify')->xss_clean(clean($meta->getAttribute('content')));
                }
                if ($meta->getAttribute('property') == 'og:by') {
                    $data['by'] = app('purify')->xss_clean(clean($meta->getAttribute('content')));
                }
                if ($meta->getAttribute('property') == 'og:status') {
                    $data['status'] = app('purify')->xss_clean(clean($meta->getAttribute('content')));
                }
                if ($meta->getAttribute('property') == 'og:on') {
                    $data['on'] = app('purify')->xss_clean(clean($meta->getAttribute('content')));
                }
            }
        }
...
function randomPassword($alphabet = false, $len = 8) {
